...
// Importações do seu Banco de Dados e Serviços
        ...
        <View style={styles.acoesContainer}>
          <View style={styles.actionsBox}>
            <TouchableOpacity
              ...
              disabled={isInativo}
            >
              <Icon name={isVencido ? "alert-circle" : "check-circle"} size={16} color="#FFF" />
            ...
            <TouchableOpacity style={styles.btnDelete} onPress={() => confirmarExclusao(item.id, item.nome)}>
              <Icon name="trash-2" size={26} color="#FF3B30" />
            </TouchableOpacity>
          </View>
        </View>
      </TouchableOpacity>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.btnAdd} onPress={() => navigation.navigate('Cadastro')}>
          <Text style={styles.btnAddText}>+ Novo Aluno</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.searchContainer}>
        ...
        {busca.length > 0 && (
          <TouchableOpacity onPress={() => setBusca('')}>
            <Icon name="x-circle" size={24} color="#999" />
          </TouchableOpacity>
        )}
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContentSmall}>
            ...
            <View style={styles.modalRowButtons}>
              <TouchableOpacity style={styles.modalBtnCancel} onPress={() => setModalPagamentoVisivel(false)}>
                <Text style={styles.modalBtnCancelText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.modalBtnSave} onPress={salvarPagamento}>
...
const styles = StyleSheet.create({
  ...
  btnAdd: { backgroundColor: '#28a745', padding: 12, borderRadius: 8, elevation: 2 },
  btnAddText: { color: '#FFF', fontWeight: 'bold', fontSize: 18 },
  
  // --- BARRA DE PESQUISA AMPLIADA ---
  searchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', margin: 15, marginBottom: 5, paddingHorizontal: 15, borderRadius: 10, borderWidth: 1, borderColor: '#E0E0E0' },
  searchIcon: { marginRight: 10 },
  searchInput: { flex: 1, paddingVertical: 15, fontSize: 20, color: '#333' },
  
  switchContainer: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', paddingHorizontal: 15, marginBottom: 5 },
  switchLabel: { fontSize: 16, color: '#666', marginRight: 10, fontWeight: '500' },
  list: { padding: 15 },

  // --- CARTÃO DO ALUNO ---
  card: { backgroundColor: '#FFF', padding: 20, borderRadius: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15, elevation: 3 },
  ...
  tagInativo: { fontSize: 14, color: '#FF3B30', fontWeight: 'bold' },

  // --- FONTES DO NOME E CPF AMPLIADAS ---
  nome: { fontSize: 26, fontWeight: '800', color: '#222' }, 
  subtext: { color: '#555', fontSize: 18, marginTop: 4, fontWeight: '500' }, 

  // --- BOTÕES DENTRO DO CARTÃO AMPLIADOS ---
  btnPagamento: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 12, borderRadius: 8, marginRight: 10 },
  btnPagtoVencido: { backgroundColor: '#FF3B30' },
  btnPagtoEmDia: { backgroundColor: '#28a745' },
  btnTextPagto: { color: '#FFF', fontSize: 15, fontWeight: 'bold', marginLeft: 6 },

  btnEdit: { backgroundColor: '#007bff', paddingHorizontal: 16, paddingVertical: 12, borderRadius: 8, marginRight: 10 },
  btnText: { color: '#FFF', fontSize: 15, fontWeight: 'bold' },
  
  btnDelete: { padding: 10 }, // Área de toque maior para a lixeira
  
  empty: { textAlign: 'center', marginTop: 50, color: '#999', fontSize: 18 },
  btnVoltar: { padding: 20, alignItems: 'center', backgroundColor: '#333' },
  btnVoltarText: { color: '#FFF', fontWeight: 'bold', fontSize: 18, letterSpacing: 1 },

  // --- ESTILOS DOS MODAIS ---
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 20 },
  modalContent: { backgroundColor: '#FFF', width: '100%', maxWidth: 550, maxHeight: '85%', borderRadius: 15, padding: 25, elevation: 10 },
  modalHeader: { borderBottomWidth: 1, borderBottomColor: '#EEE', paddingBottom: 15, marginBottom: 15 },
  modalTitle: { fontSize: 26, fontWeight: 'bold', color: '#000' },
  modalSubtitle: { fontSize: 16, color: '#666', marginBottom: 20 },
  modalStatus: { fontSize: 16, fontWeight: 'bold', marginTop: 5 },
  modalSection: { marginBottom: 15 },
  modalLabel: { fontSize: 12, color: '#666', fontWeight: 'bold', marginBottom: 4 },
  modalDado: { fontSize: 18, color: '#333', marginBottom: 10 },

  modalSectionDestacada: { backgroundColor: '#F9F9F9', padding: 15, borderRadius: 10, marginTop: 10, marginBottom: 10 },
  linhaPagamento: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#EEE', paddingVertical: 12 },
  modalDadoPagtoData: { fontSize: 18, fontWeight: 'bold', color: '#333' },
  modalDadoPagtoValor: { fontSize: 18, fontWeight: 'bold', color: '#28a745' },

  modalBtnFechar: { backgroundColor: '#333', padding: 18, borderRadius: 8, alignItems: 'center', marginTop: 15 },
  modalBtnText: { color: '#FFD700', fontWeight: 'bold', fontSize: 18, letterSpacing: 1 },

  modalContentSmall: { backgroundColor: '#FFF', width: '100%', maxWidth: 400, borderRadius: 15, padding: 25, elevation: 10 },
  modalLabelInput: { fontSize: 14, color: '#333', fontWeight: 'bold', marginBottom: 8 },
  modalInput: { backgroundColor: '#F0F0F0', borderRadius: 8, padding: 18, fontSize: 22, color: '#000', textAlign: 'center', marginBottom: 20 },
  modalRowButtons: { flexDirection: 'row', justifyContent: 'space-between' },
  modalBtnCancel: { flex: 1, backgroundColor: '#EEE', padding: 18, borderRadius: 8, alignItems: 'center', marginRight: 10 },
  modalBtnCancelText: { color: '#333', fontWeight: 'bold', fontSize: 16 },
  modalBtnSave: { flex: 1, backgroundColor: '#28a745', padding: 18, borderRadius: 8, alignItems: 'center' },
